...
export async function POST(request: Request) {
  try {
    await connectDB();
    const { name, password, id, email, program, phone, campus, level, faculty, session } =
      await request.json();
    ...
    const newUser = new StudentModel({
      name, id, email, password: hashedPassword, program, phone, campus, level, session, faculty,
    });
